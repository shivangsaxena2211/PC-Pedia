"""Generate verified_intel_6th_gen.py and verified_intel_7th_gen.py from ARK fetch output."""
...
ARK_DATA = Path(__file__).resolve().parent / "ark_desktop_6_7.json"
...
def convert_entry(entry: dict, generation: str) -> dict:
    ...
    specs = dict(COMMON_7 if generation == "7th" else COMMON_6)
...
def main():
    ...
    gen7 = sorted(
        [convert_entry(e, "7th") for e in data if "7th" in e["collection"]],
        key=lambda x: x["model"],
    )
    gen6 = sorted(
        [convert_entry(e, "6th") for e in data if "6th" in e["collection"]],
        key=lambda x: x["model"],
    )
    (OUT_DIR / "verified_intel_7th_gen.py").write_text(
        render_module("7th Gen", "INTEL_7TH_GEN_DESKTOP", gen7),
        encoding="utf-8",
    )
    (OUT_DIR / "verified_intel_6th_gen.py").write_text(
        render_module("6th Gen", "INTEL_6TH_GEN_DESKTOP", gen6),
        encoding="utf-8",
    )
    print(f"Wrote {len(gen7)} 7th gen and {len(gen6)} 6th gen entries")
